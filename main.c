...
//Disable karena soal minta delete dengan Index
...
void sort_books_by_name_bubble_asc();
void sort_books_by_price_selection_desc();
  ...
  while (1)
  {
    ...
    switch (choice){
      ...
      case 6: {
        ...
        if (BOOKS) free(BOOKS);
        if (SALES_RECORDS) free(SALES_RECORDS);
...
int read_int(const char *prompt, int *outValue) {
  ...
  if (sscanf(line, "%d %c", &value, &extra) != 1) return 0;
...
char *generate_book_code(){
  ...
  if (lastCode != NULL){
    number = atoi(lastCode + 2) + 1; //+2, +3 got error
  }
...
//SORT
Book *clone_books(const Book *source, size_t count){
  if (count == 0 || source == NULL) return NULL;
